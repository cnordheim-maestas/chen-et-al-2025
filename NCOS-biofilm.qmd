...
fig2b_summary %>% 
  ...
  mutate(sample_ID = factor(sample_ID, 
    ...
    sample_ID == "1%TB" ~ "dotted",
    sample_ID == "MQ" ~ "dotted",
    sample_ID == "1%TB+PW+microorganism" ~ "solid",
    sample_ID == "PW+microorganism" ~ "solid",
    sample_ID == "1%TB+PW-microorganism" ~ "dotted",
    sample_ID == "PW-microorganism" ~ "dotted",
    sample_ID == "Added Bd" ~ "solid",
    TRUE ~ "solid"  # Default case, if any
  )) %>% 
  ...
  geom_errorbar(aes(ymin = mean - se, # plot the standard error
                    ymax = mean + se),
                width = 0.1) +
    geom_line(aes(linetype = line_type), show.legend = FALSE) + 
