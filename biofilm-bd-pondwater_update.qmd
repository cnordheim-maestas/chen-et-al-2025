...
ns_wider_combined <- ns_ml_pw_bd %>%
  pivot_wider(names_from = bd_location, values_from = bd_qty) %>%
  mutate(combined_bd = adherent + supernatant)

ns_wider_combined <- subset(ns_wider_combined, select = -c(adherent,supernatant) )

ns_wider_combined <- ns_wider_combined %>% 
  # calculate diff
  pivot_wider(names_from = day, values_from = combined_bd) %>%
  mutate(diff = Day_1 - Day_7)

# Step 2: create subsets for each treatment
ns_wider_combined_40um <- ns_wider_combined %>% 
  filter(filter =="40um_filter")
ns_wider_combined_.22um <- ns_wider_combined %>% 
  filter(filter =="0.22um_filter")
...
# check normality of the differences across groups
ns_wider_combined_40um %>% 
 ggqqplot("diff", title = "40 um") # not great
shapiro.test(ns_wider_combined_40um$diff) # NOT normal
hist(ns_wider_combined_40um$diff) # nope

# we gotta transform, but we have negative: west GAINED Bd
ns_wider_combined_40um <- ns_wider_combined_40um %>% 
  mutate(log_diff = log(diff))

ns_wider_combined_40um %>% 
  ggqqplot("log_diff", title = "40 um") # good enough
shapiro.test(ns_wider_combined_40um$log_diff) # woohoo!!
hist(ns_wider_combined_40um$log_diff) # sure

# 0.22 micron

# check normality of the differences across groups
ns_wider_combined_.22um %>% 
 ggqqplot("diff", title = ".22 um") # not great
shapiro.test(ns_wider_combined_.22um$diff) # shapiro says its fine
hist(ns_wider_combined_.22um$diff) # meh

# we gotta transform, but we have negative: west GAINED Bd
ns_wider_combined_.22um <- ns_wider_combined_.22um %>% 
  mutate(log_diff = log(diff))

ns_wider_combined_.22um %>% 
  ggqqplot("log_diff", title = ".22 um") # better?
shapiro.test(ns_wider_combined_.22um$log_diff) # woohoo!!
hist(ns_wider_combined_.22um$log_diff) # sure
...
```{r}
# Step 3: run the paired t-test on the difference
t.test(ns_wider_combined_40um$log_diff, ns_wider_combined_.22um$log_diff, paired = TRUE)
```
