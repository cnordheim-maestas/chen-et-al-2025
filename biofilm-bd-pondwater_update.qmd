...
summary(glht(lme_mod, linfct=mcp(day="Tukey"))) # same output

```

If this is correct (and note I saw a similar trend when I did the pairwise ANOVA), then the result is that there is an overall effect of Day on the amount of Bd detected in the field biofilm (p > 0.001) and that Day_0 had less Bd in the biofilm than Day_1 (<.0001), Day_1 had less Bd in the biofilm than day_7 (<.0001), and there was no difference in the amount of the Bd on day 0 and day 7 (p=0.42). The test is a linear mixed effects model (the model is the log transformed Bd quantity ~ day, random = ~1|site) with a post hos pairwise comparison using the emmeans package. The initial model is recommended for a randomized block design, day is the fixed effect and site is the random effect. What I am not as sure about is the use of the emmeans package for the post-hoc comparisons so don't take these to the bank yet. I saw this used here: https://www.researchgate.net/post/Post_hoc_test_in_linear_mixed_models_how_to_do. 
...
ggplot(aes(y= combined_bd, x = filter, fill = filter)) + 
  ...
  scale_x_discrete (labels= c("40um_filter" = "With Microbes", "0.22um_filter" = "No Microbes")) +
    xlab("Presence of Microbes in Pond Water") +
    ylab("Bd Quantity \n (zoospore equivalents)") +
  ggtitle("Monolayer Bd Biofilm Exposed to Pond Water")
...
When monolayer Bd biofilm is exposed to pond water without microbes has a significantly greater loss of Bd between day 1 to day 7 than pond water with microbes (t = -6.5, df = 7, p = 0.0003491). The t-value is positive, which shows that the first item entered (with microbes) has a smaller loss of Bd than the second item entered (no microbes).
...
anova(mod3)
summary(mod3) # sig!
post_hoc <- glht(mod3, # with your ANOVA model
                 linfct = mcp(treatment = "Tukey"))
summary(post_hoc)
cld(post_hoc) # gives diff pairwise diff than the above
```

I am thinking myself in circles with the directional of the rate of loss... log(day1) - log(day7) gives a positive value and log(day7) - log(day1) gives a negative value... regardless...a bigger absolute value means a greater death rate, so we are seeing the same general trends BUT the post-hoc is a bit different inn between them. Note the "rate" one fits assumptions better. 
