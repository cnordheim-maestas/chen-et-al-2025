...
Let's make this a prettier table
...
anova_model <- aov(log_adh ~ day * medium, data = ae_noday0)
...
tukey_combined <- tukey_combined %>%
  mutate(
    ...
    p_value_with_significance = paste0(p.adj, significance)
  )
...
tukey_combined %>%
  dplyr::select(factor, comparison, diff, lwr, upr, p_value_with_significance) %>%
  gt() %>%
  cols_label(
    factor = "Comparison",
    ...
    lwr = "Lower CI",
    upr = "Upper CI",
    p_value_with_significance = "p-value"
  ) %>%
  tab_header(
