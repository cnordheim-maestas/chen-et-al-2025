...
ggplot(aes(y= bd_qty, x = day)) + 
  ...
  scale_y_log10() +
   theme(legend.position = "none",
    ...
    xlab("Time (days)") +
    ylab("Bd Quantity \n (zoospore equivalents)") +
  scale_x_discrete (labels= c("Day_0" = "Day 0", "Day_1" = "Day 1", "Day_7" = "Day 7"))

# Let's add significance letters
significance_data <- tibble(
 day = factor(c("Day_0", "Day_1", "Day_7"), levels = c("Day_0", "Day_1", "Day_7")),
  y_position = c(1.4e+04, 1.6e+05, 1.4e+04),  # Adjust this depending on your plot's scale
  label = c("a", "b", "a"))
...
```{r}
all_parts$filter <- factor(all_parts$filter)
ap <- all_parts %>% 
  ggplot(aes(y= rate_loss, x = treatment, fill = filter)) + 
    geom_boxplot() +
  geom_jitter(alpha = 0.5, width = 0.1) +
   ...
   theme(legend.position = "none",
         ...
         axis.title = element_text(face = "bold")) +
    xlab("Treatment") +
      ylab("Death rate of Bd over 7 Days")

ap + geom_text(data = sig_data_rl, aes(x = treatment, y = y_position, label = label), inherit.aes = FALSE) 
```
...
### another way to organize th same vis
...
# Appendix and ramblings

I'm also thinking a 2 way anova may be more appropriate since we have 2 aspects of "treatments" here, the filter and the monolayer vs pond water vs field biofilm. Or, we can just look at the "no microbes" across the treatments? But the comparisons with the monolayer are also interesting...

## two way anova cause why not?

Well, we don't have each pair to compare, and I think we may be messing with the power of the test, so perhaps this wasn't wise, but I will leave it here and think on it!

```{r}
all_parts_with_medium <- all_parts %>% 
  mutate(medium = case_when(
    grepl("PW", treatment) ~ "PW",
    grepl("Field bf", treatment) ~ "Field bf",
    grepl("Monolayer", treatment) ~ "Monolayer"
...
all_parts_with_medium$medium <- as.factor(all_parts_with_medium$medium)
all_parts_with_medium$filter <- as.factor(all_parts_with_medium$filter)


mod4 <- aov(rate_loss~medium*filter, data=all_parts_with_medium)
plot(mod4) # not bad
anova(mod4)
summary(mod4) # sig!
TukeyHSD(mod4)
