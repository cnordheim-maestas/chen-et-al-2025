...
-   5: ANOVA and Tukey (or glm if needed)
...
# calculate the rate loss by taking the log of each before subtracting
  ...
  mutate(rate_loss = log(Day_1) - log(Day_7)) %>% 
  dplyr::select(sample_ID, diff, treatment, rate_loss, Day_1)
# Note: NCOS AE with pond water and microbes does not exist!
  
all_parts<- bind_rows(pw_WITH_microbes, pw_no_microbes, fieldbf_no_microbes, ncos_pw_no_microbes, ncos_pw_WITH_microbes, ncos_ae_no_microbes) %>% #, monolayer_WITH_microbes, monolayer_NO_microbes
  mutate(percent_loss = (diff/Day_1) * 100) 

all_parts2 <- bind_rows(pw_WITH_microbes, fieldbf_no_microbes, ncos_pw_WITH_microbes, ncos_ae_no_microbes) %>% #, monolayer_WITH_microbes, monolayer_NO_microbes
  mutate(percent_loss = (diff/Day_1) * 100) 

# set PW 40 um as the intercept, comparing back to that one
all_parts$treatment <- factor(all_parts$treatment,
                          levels = c("PW 0.22um", "NCOS PW 0.22um", "PW 40um", "NCOS PW 40 um", "Field bf 0.22", "NCOS AE 0.22um" )) #, "Monolayer 40um", "Monolayer 0.22um"

all_parts2$treatment <- factor(all_parts2$treatment,
...
# without microbe depleted
par(mfrow = c(2,2))
mod4 <- aov(rate_loss~treatment, data=all_parts2)
plot(mod4)
...
sig_data_rl <- tibble(
  ...
  y_position = c(1.5,2.5,6.5,4, 6.5,4),
  label = labels_rl)

# without microbe depleted ###
summary(mod4) # sig effect of treatment
TukeyHSD(mod4)

post_hoc_rateloss2 <- glht(mod4, # with your ANOVA model
                 linfct = mcp(treatment = "Tukey"))
summary(post_hoc_rateloss2)
cld(post_hoc_rateloss2) 

# export cld results as a dataframe
cld_results_rateloss2 <- cld(post_hoc_rateloss2)
labels_rl2 <- cld_results_rateloss2$mcletters$Letters
treatments_rl2 <- names(labels_rl2)
labels_rl2 <- as.character(labels_rl2)

sig_data_rl2 <- tibble(
  treatment = treatments_rl2,
...
ab <- all_parts %>% 
  mutate(medium = case_when(
    ...
    grepl("0.22", treatment) ~ "no microbe",
    grepl("40", treatment) ~ "microbes"
  )) %>% 
   ggplot(aes(y= rate_loss, x = treatment, fill = filter)) + 
    geom_boxplot() +
  geom_point(alpha = 0.5) +
    theme_classic() +
   scale_fill_manual(values = c("microbes" = with_microbes_40_color, 
                                  "no microbe" = no_microbes_.22_color)) +
   theme(legend.position = "none",
         strip.text = element_text(face="bold"),
         axis.title = element_text(face = "bold")) +
  xlab("Medium") +
  ylab("Rate loss of Bd over 6 Days") + 
  scale_x_discrete (labels= c("PW 0.22um" = "EB PW - MO"
...
mutate(filter = case_when(
    grepl("40", treatment) ~ "40um_filter",
...
ab + geom_text(data = sig_data_rl, aes(x = treatment, y = y_position, label = label), inherit.aes = FALSE) 
```

```{r}
all_parts2$treatment <- factor(all_parts2$treatment,
                          levels = c("PW 40um", "Field bf 0.22", "NCOS PW 40 um", "NCOS AE 0.22um")) 
...
ab2 <- all_parts2 %>% 
  ...
  geom_jitter(alpha = 0.5, width = 0.05) +
    theme_classic() +
   # scale_fill_manual(values = c("PW" = "pink", 
   #                                "Field bf" = "orange")) +
   theme(legend.position = "none",
         strip.text = element_text(face="bold"),
         axis.title = element_text(face = "bold")) +
  ...
  scale_x_discrete (labels= c("PW 0.22um" = "EB PW - MO"
                              ...
                              "Field bf 0.22" = "EB Biofilm",
                              "NCOS AE 0.22um" = "SB Biofilm")) +
  ggtitle("Option 4: reordered, with no color, no facet")

sig_data_rl2 <- sig_data_rl2 %>% 
mutate(filter = case_when(
    grepl("40", treatment) ~ "40um_filter",
    grepl("0.22", treatment) ~ "0.22um_filter")) %>% 
mutate(medium = case_when(
...
ab2 + geom_text(data = sig_data_rl2, aes(x = treatment, y = y_position, label = label), inherit.aes = FALSE) 
```
...
ab2 <- all_parts2 %>% 
  ...
  # biofilm and pond water colors, both from paul tol colorblind palette
   scale_fill_manual(values = c("PW" = "#eedd88",
                                  "Field bf" = "#ee8866")) +
  ...
  scale_x_discrete (drop = TRUE, labels= c("PW 0.22um" = "EB PW - MO",
                              ...
                              "Field bf 0.22" = "EB Biofilm",
                              "NCOS AE 0.22um" = "SB Biofilm")) +
  ggtitle("Option 3: colored by biofilm/pondwater, facet by site")
...
ab2 + geom_text(data = sig_data_rl2, aes(x = treatment, y = y_position, label = label, group = site), inherit.aes = FALSE) 

```

## STATSBONUS test broader biofilm
...
par(mfrow = c(2,2))
mod5 <- aov(rate_loss~medium, data=ap3)
plot(mod5)
...
leveneTest(rate_loss ~ treatment, data = ap3)
