...
# 2A: East Bay pond water microbes
...
f4b_em_interaction <- emmeans(aov_4b, ~ microbes * TB)
f4b_tukey_interaction <- pairs(f4b_em_interaction, adjust = "tukey")
f4b_tukey_interaction 
```
...
```{r}
f5a <- all_parts %>% 
  mutate(medium = fct_relevel(medium, "PW", "Field bf")) %>% 
...
fig5 <- all_parts %>% 
  ...
  scale_x_discrete (drop = TRUE, labels= c(
                              "PW 40um" = "East Bay\nPW + MO",
                              "NCOS PW 40 um" = "SB \nPW + MO",
                              "Field bf 0.22" = "East Bay\nBiofilm",
                              "NCOS AE 0.22um" = "SB \nBiofilm"))
...
f4b_tukey_interaction_df <- as.data.frame(f4b_tukey_interaction)

# Add labels to indicate which factor the comparison refers to
f4b_tukey_day_df  <- f4b_tukey_day_df  %>% mutate(factor = "Day")
f4b_tukey_microbes_df <- f4b_tukey_microbes_df %>% mutate(factor = "Medium")
f4b_tukey_interaction_df <- f4b_tukey_interaction_df %>% mutate(factor = "Interaction")

f4b_all_tukey_df <- bind_rows(f4b_tukey_day_df, f4b_tukey_microbes_df, f4b_tukey_interaction_df)
