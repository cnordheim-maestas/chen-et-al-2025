...
scale_y_log10(labels = function(x) {gsub("e", "E", scales::scientific_format()(x))}, 
                  limits = c(1e+02, 1e+06)) +  # Set y-axis range from 1e2 to 1e3
    facet_wrap(~day, labeller = labeller(day = c("Day_1" = "Day 1",
                                                    "Day_7" = "Day 7")))+
  scale_fill_manual(values = c("40um_filter" = with_microbes_40_color, 
                                "0.22um_filter" = no_microbes_.22_color)) +
...
ggsave("4a_updated.pdf", plot = fig_4a, width = 3.46, height = 3.46)
```

 Fig 4 Renwei scale

```{r}
fig_4a_RS <- eb_monolayer %>% 
 mutate(filter = factor(filter, levels = c("40um_filter", "0.22um_filter"))) %>% # Switch the order of levels
  # plot it
  ggplot(aes(y= bd_qty, x = filter, fill = filter)) + 
    geom_boxplot() +
...
# 4a alone
#ggsave("4a.pdf", plot = fig_4a, width = 3.46, height = 3.46)

#ggsave("fig_4a_RS.pdf", plot = fig_4a_RS, width = 3.46, height = 3.46)

# 4b alone
#ggsave("4b.pdf", plot = fig_4b, width = 3.46, height = 3.46)
```
...
#ggsave("SI_4b.pdf", plot = pairwise_cld_4b, width = 7.09, height = 3.46)
...
combined_data_4 <- bind_rows(
  eb_monolayer_nolog,
  eb_ml_controls) # controls data is already in the required format
...
ggplot(aes(y= bd_qty, x = site, fill = filter)) + 
  ...
  scale_y_continuous(expand = c(0.01, 0.01),
   ...
   scale_x_discrete(limits = c("CABIN", "GRAMPS", "WEST", "GDPND005", "GDPND006", "GDPND009", "PRPND004", "PRPND009", "PRPND010", "sterile MQ")) +  
...
ggsave("SI_4a.pdf", plot = SI_4a, width = 7.09, height = 3.46)
```
